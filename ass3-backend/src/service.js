...
export const assertWatcherOfJobPost = (userId, jobPostId) => {
  ...
  const { usersWhoWatchMeUserIds } = users[creatorId];
  if (!Object.keys(usersWhoWatchMeUserIds).includes(userId)) {
...
export const getJobs = (authUserId, start) => dataLock((resolve, reject) => {
  ...
  const relevantPosts = allPosts.filter(p => Object.keys(users[p.creatorId].usersWhoWatchMeUserIds).includes(authUserId));
  const expandedPosts = relevantPosts.map(buildJobObject);
  expandedPosts.sort((a, b) => (a.createdAt < b.createdAt) ? 1 : -1)
  const nextJobs = expandedPosts.slice(start, start + 5);
  resolve(nextJobs.map(j => ({
    ...j,
    userId: parseInt(j.id, 10),
  })));
});
...
export const postJob = (authUserId, image, title, start, description) => dataLock((resolve, reject) => {
  if (image === undefined || title === undefined || start === undefined || description === undefined) {
    return reject(new InputError('Please enter all relevant fields'));
  }
  const newJob = {
    id: newJobId(),
...
export const getUser = (userId) => dataLock((resolve, reject) => {
  ...
  const userDetails = {
    ...
    password: undefined,
    id: intid,
    usersWhoWatchMeUserIds: Object.keys(users[userId].usersWhoWatchMeUserIds).map(i => parseInt(i, 10)),
...
export const watchUser = (watcherUserId, watcheeUserId, turnon) => dataLock((resolve, reject) => {
  ...
  if (turnon) {
    userDetails.usersWhoWatchMeUserIds[watcherUserId] = true;
  } else {
    if (Object.keys(userDetails.usersWhoWatchMeUserIds).includes(watcherUserId)) {
      delete userDetails.usersWhoWatchMeUserIds[watcherUserId];
    }
  }
  resolve();
});
